...
export default function NeuralBackground() {
  ...
  React.useEffect(() => {
    ...
    gsap.set(containerRef.current, { filter: 'blur(20px)', scale: 1.02, autoAlpha: 0.8 });
    gsap.to(containerRef.current, { filter: 'blur(0px)', scale: 1, autoAlpha: 1, duration: 1.2, ease: 'power3.out', delay: 0.2 });
  }, []);

  return (
    <div ref={containerRef} style={{ position: 'absolute', inset: 0, zIndex: 0, background: '#ffffff' }} aria-hidden>
      <Canvas camera={camera} gl={{ antialias: true, alpha: true }} dpr={[1, 2]} style={{ width: '100%', height: '100%' }}>
        <ShaderPlane />
      </Canvas>
      <div style={{ pointerEvents: 'none', position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(139,92,246,0.25) 0%, rgba(59,130,246,0.15) 40%, rgba(255,255,255,0) 80%)' }} />
      <div style={{ pointerEvents: 'none', position: 'absolute', inset: 0, background: 'radial-gradient(1200px 600px at 0% 0%, rgba(139,92,246,0.15), rgba(255,255,255,0)), radial-gradient(1200px 600px at 100% 100%, rgba(59,130,246,0.12), rgba(255,255,255,0))' }} />
    </div>
  );
}
